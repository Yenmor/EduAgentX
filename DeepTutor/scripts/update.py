#!/usr/bin/env python
"""Safely update a local EduAgentX git checkout.
...
def run_update(repo_root: Path, *, assume_yes: bool) -> int:
    ...
    print("Restart EduAgentX if it is currently running.")
    return 0


def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fetch, review, and fast-forward update a local EduAgentX checkout."
    )
    parser.add_argument(
        "--yes",
        "-y",
    ...
    parser.add_argument(
        ...
        help="Path to the git checkout to update. Defaults to this EduAgentX repository.",
    )
