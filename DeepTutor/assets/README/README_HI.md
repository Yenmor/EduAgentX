...
# EduAgentX: लाइफलॉन्ग व्यक्तिगत ट्यूटरिंग
...
> 🤝 **हम किसी भी प्रकार के योगदान का स्वागत करते हैं!** [`Roadmap`](https://github.com/HKUDS/EduAgentX/issues/498) पर roadmap items के लिए वोट करें या नए प्रस्तावित करें, और branching strategy, coding standards और शुरू करने के तरीके के लिए हमारी [Contributing Guide](../../CONTRIBUTING.md) देखें।

### 📰 समाचार

- **2026-05-22** 🌐 आधिकारिक डॉक्स साइट [**eduagentx.local**](https://eduagentx.local/) पर live — guides, references, और capability tours एक ही जगह।
...
- **2026-04-10** 📄 हमारा paper अब arXiv पर live है — EduAgentX के design और विचारों के लिए [preprint](https://arxiv.org/abs/2604.26962) पढ़ें।
- **2026-02-06** 🚀 39 दिनों में 10k स्टार्स! हमारे अविश्वसनीय community के समर्थन के लिए बहुत धन्यवाद।
- **2026-01-01** 🎊 नया साल मुबारक! हमारे [Discord](https://discord.gg/eRsjPgMU4t), [WeChat](https://github.com/HKUDS/EduAgentX/issues/78), या [Discussions](https://github.com/HKUDS/EduAgentX/discussions) से जुड़ें — आइए मिलकर EduAgentX को आकार दें।
- **2025-12-29** 🎓 EduAgentX आधिकारिक रूप से जारी हुआ!

## ✨ मुख्य विशेषताएं

EduAgentX एक agent-native learning workspace है जो tutoring, problem solving, quiz generation, research, visualization, और mastery practice को एक extensible system में जोड़ता है।
...
`next_public_api_base_external` (और इसका alias `public_api_base`) lower-precedence fallbacks के रूप में accept किए जाते हैं। CORS frontend **origins** उपयोग करता है, API URLs नहीं। Auth disabled होने पर, EduAgentX default रूप से normal HTTP/HTTPS browser origins permit करता है। Auth enabled होने पर, exact frontend origins add करें:
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/OVERVIEW.png" alt="EduAgentX home — sidebar में हर surface के साथ Chat workspace" width="900">
</div>
...
My Agents दूसरे agents को EduAgentX के लिए context बनाता है, और दो अलग काम करता है। **लाइव एजेंट connect करें** — आपकी machine पर Claude Code या Codex CLI, या आपके Partners में से एक — और इसे chat turn के अंदर से consult करें: EduAgentX actually दूसरे agent को *run* करता है और इसके काम को `consult_subagent` tool के जरिए Activity panel में stream करता है। इसे Agent chip से select करें (या `@` type करें), और set करें कि consult कितने rounds ले सकता है।
...
**पिछली conversations import करें** — अपनी existing Claude Code और Codex history को named, searchable, resumable agents के रूप में bring in करें। Import करने के लिए कौन से days लेने हैं चुनें; refreshing उन्हें re-sync करता है। किसी भी chat turn से imported conversation को `+` → My Agents के जरिए reference करें, और EduAgentX इसे एक third-party transcript के रूप में पढ़ता है — यह उनकी conversation रहती है, EduAgentX की अपनी आवाज नहीं।
...
Authentication **default रूप से बंद** है — EduAgentX single-user चलता है। इसे on करें और एक `data/` tree एक admin workspace, isolated per-user workspaces, और partner workspaces को side by side host करती है:
...
## ⌨️ EduAgentX CLI — एजेंट-नेटिव इंटरफेस

एक `deeptutor` binary, दो तरीके से: terminal में रहने वालों के लिए interactive **REPL**, और EduAgentX को tool के रूप में drive करने वाले दूसरे agents के लिए structured **JSON**। दोनों तरफ same capabilities, tools, और knowledge bases।
...
EduAgentX skills open **Agent-Skills** format उपयोग करती हैं — एक `SKILL.md` playbook (YAML frontmatter + Markdown) और optional reference files के साथ एक folder। इसमें EduAgentX-specific कुछ नहीं है, इसलिए format बोलने वाली कोई भी registry आपकी library के लिए एक source बन जाती है। EduAgentX **[EduHub](https://eduhub.eduagentx.local/)** के साथ ship होता है — हमारी अपनी education-focused skill registry — default hub के रूप में built in।

<details>
<summary><b>EduHub — EduAgentX का skill ecosystem</b></summary>

[**EduHub**](https://eduhub.eduagentx.local/) वह community hub है जिसे EduAgentX ने teaching-oriented agent skills share करने के लिए launch किया — Socratic tutors, flashcard builders, essay feedback, exam blueprints, concept explainers, और बहुत कुछ। यह EduAgentX में built in है, इसलिए configure करने की कोई जरूरत नहीं: एक bare slug या `eduhub:` prefix इसे resolve करता है।
...
EduHub एक standalone, ClawHub-compatible registry भी है, इसलिए EduAgentX नहीं होने वाले agents (Claude Code, Codex, …) इसे `eduhub` CLI के जरिए directly use कर सकते हैं — `npx eduhub install socratic-tutor`।
...
- frontmatter EduAgentX के schema में normalize होता है और `always:` **stripped** होता है, इसलिए एक downloaded skill खुद को हर system prompt में force नहीं कर सकती;
...
क्योंकि EduAgentX open Agent-Skills format बोलता है, **[ClawHub](https://clawhub.ai/)** भी एक first-class source है — यह EduHub के साथ built in है। इसे hub prefix से चुनें:
...
| [**Claude Code**](https://github.com/anthropics/claude-code) | Agentic coding CLI जिसने EduAgentX agent loop को inspire किया |
| [**ManimCat**](https://github.com/Wing900/ManimCat) | Math Animator के लिए AI-driven math animation generation |

### 🗺️ Roadmap और योगदान

हम चाहते हैं कि EduAgentX iterate और improve करता रहे — और अंततः open-source community को एक gift बने। हमारा [**roadmap**](https://github.com/HKUDS/EduAgentX/issues/498) continuously update होता है; वहां items पर vote करें या नए propose करें। अगर आप contribute करना चाहते हैं, तो branching strategy, coding standards, और शुरू करने के तरीके के लिए [**Contributing Guide**](../../CONTRIBUTING.md) देखें।

<div align="center">

हम आशा करते हैं कि EduAgentX community के लिए एक उपहार बने। 🎁
